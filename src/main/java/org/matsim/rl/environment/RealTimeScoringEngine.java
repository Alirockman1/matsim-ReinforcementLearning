...
public class RealTimeScoringEngine {
    ...
    public RealTimeScoringEngine(Scenario scenario, Logger log){
    ...
    public void compute(MobsimAgent agent, double currentTime, Activity activity, String mode, Trip trip, double modeRetrievalTime, int numberOfTransfers, Map<String, Integer> modeDiscontinuityPenaltyMap){
        ...
        if (populationScoringParameters == null) return;
        ...
        // Get penalty weights
        RLConfigGroup rlConfig = ConfigUtils.addOrGetModule(localScenario.getConfig(), RLConfigGroup.class);
        double discontinuityWeight = rlConfig.getModelWeights().get("discontinuityPenalty");
        double retrievalCostWeight = rlConfig.getModelWeights().get("retrievalCostPenalty");
